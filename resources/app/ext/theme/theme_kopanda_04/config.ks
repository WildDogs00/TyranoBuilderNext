...
;=========================================
	...
	/* 画像類のパス */
	tf.img_path          = '../others/plugin/theme_kopanda_04/image/config/';
	tf.btn_path_off      = tf.img_path + 'c_btn.gif';
	tf.btn_path_on       = tf.img_path + 'c_set.gif';
	tf.img_path_skip_on  = tf.img_path + 'c_uts_on.gif';
	tf.img_path_skip_off = tf.img_path + 'c_uts_off.gif';

	// チェック画像の座標
	tf.config_x = [800, 300, 345, 390, 435, 480, 525, 570, 615, 660, 705];
	tf.config_y = [160, 210, 280, 330];
...
;	画面右上の「Back」ボタン
	[button fix="true" graphic="&tf.img_path+'c_button_back.png'" enterimg="&tf.img_path+'c_button_back2.png'" target="*backtitle" x="880" y="10"]
...
;------------------------------------------------------------------------------------------------------
;	未読スキップ-OFF
	[button name="unread_off" fix="true" target="*skip_off" graphic="&tf.btn_path_off" width="130" height="40" x="300" y="400"]

;	未読スキップ-ON
	[button name="unread_on"  fix="true" target="*skip_on"  graphic="&tf.btn_path_off" width="130" height="40" x="440" y="400"]
...
*load_skip_img
	...
	[image layer="0" name="skip" storage="&tf.img_path_skip_on"  x="440" y="400"]
	[else]
	[image layer="0" name="skip" storage="&tf.img_path_skip_off" x="300" y="400"]
	[endif]
	[return]
